...
{
  XboxController::data.update(pData, length);
}
...
void mainLoop(void *pt)
{
  while (true)
  {
    if (!XboxController::connected)
    {
      ...
      else if (!XboxController::scanning)
      {
        startScan();
      }
    }
...
bool XboxController::scanning = false;
bool XboxController::connected = false;
